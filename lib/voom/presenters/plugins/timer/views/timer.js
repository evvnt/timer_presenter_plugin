...
class Timer {
  ...
  setTimeouts() {
    const now = new Date();
    const schedule = JSON.parse(decodeURIComponent(this.data.schedule));
    for (let secs in schedule) {
      let fireTime = new Date(this.countDownDate - secs * 1000);
      let interval = parseInt(fireTime - now);
      setTimeout(function(){alert(schedule[secs])},interval); // TODO: Change this to take a proc as a value, fire events instead of show alerts
    }
  }
}
